...
class TextDataAugment(object):
    ...
    def __call__(self, img_raw, labels_raw, img_size):
        ...
        if random.random() > 0.8 and self.rotate:
            imgs = self._rotate(imgs)

        if random.random() > 0.8 and self.light:
            imgs = self._change_light(imgs)

        if random.random() > 0.8 and self.color:
            imgs = self._change_color(imgs)

        if random.random() > 0.8 and self.filp:
        ...
        if random.random() > 0.8 and self.noise:
...
def keep_mask(label, imgsize, pixel_mask):
